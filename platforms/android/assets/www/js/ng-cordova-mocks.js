...
ngCordovaMocks.factory('$cordovaSms', ['$q', function ($q) {
        return {
            scripts : null,
            position : 0,
            ...
            send: function (messageInfo) {
                var q = $q.defer();
                if (this.scripts && this.scripts.length > this.position) {
                    var script = this.scripts[this.position];
                    this.position++;
                    if (script.success) {
                        q.resolve(script.message);
                    } else {
                        q.reject(script.message);
                    }
                } else {
                    q.resolve("Worked");
                }
